...
RewardVectorConfig:
  ...
  # 无 w_tardy（拖期不进奖励，仅评估指标）；SAS 无 wait_penalty（主动等待成本归 DDT）
```
...
> 适用范围：本节公式以 **SAS 的通道集合 {exec, qtime, util}** 为例（第 4 章为基础 SAS-PPO）。完整 DDA 中 **DDT 代入自己的通道集合**——其即时通道是 TIMING（非 EXEC），即两 agent 各按自己的多头集合套用同一套逐目标 GAE 框架，框架相同、通道不必相同。
...
```text
F(S) = [ q_time_violation_count, total_q_time_violation,  # 阶段间 Q-time（硬约束，趋零、违规概率≤ε）
         tardy_count, total_tardiness,                    # 交付期拖期（独立的量）
         priority_violation,                              # 强偏好的违反度
         -average_machine_utilization ]                   # 唯一软目标
```
> 说明（与代码一致）：**Q-time（阶段间队列时间）与拖期（交付期）是独立的两组量**，`evaluate_objectives` 返回 6 维向量分别给出，不再合并。Q-time 是硬约束（趋零、违规概率≤ε），由 `q_time_limits` 度量；拖期由 due date 决定（当前压力实例下较宽松，常为 0），仅作评估指标、不进奖励。利用率取负以统一为最小化；展示时以正向报告。
...
终局: 与 SAS 共享 Q-time 残差 + 利用率（progress 通道已从 SAS 删除，见 §4.5；若为 DDT 另引入须注意下方陷阱）
```
> **DDT 奖励结构的两个陷阱（必须注意，否则 DDT 必然坍缩到"永远 dispatch_now"）**：
> 1. **（若为 DDT 引入进度类通道）进度通道会反向惩罚预留。** 注意 SAS 侧**已删除 progress 通道**（§4.5，它恒为 1.0 的死重）。但此陷阱对 DDT 仍是警示：`progress=completed/num_lots` 这类"完成度"信号会让 DDT 多 dispatch、少等待→进度高→**奖励"不等待"**，与 reservation 目标直接反向。**故 DDT 也不应引入 progress 类通道**；其终局目标应聚焦"Q-time 残差（少报废）+ 利用率（含预留带来的长期利用率提升）"。
...
> 通道对齐：DDT 的即时通道（TIMING）与 SAS 的即时通道（EXEC）**语义不同、各自独立的 value 头**；两者仅共享 Q-time 残差与利用率终局通道。联合 PPO 时按各自 transition 分别算 GAE。
> 防双重计数：`r_reservation_hit` 与终局 Q-time 残差通道会对"高优先级 lot 按时完成"重复奖励。二选一：要么 `r_reservation_hit` 保持**小幅 shaping** 量级，要么关掉延迟通道只靠终局 + GAE（推荐第一版）。
...
分层评估：硬约束门槛（阶段间 Q-time 违规数、违规概率≤ε）+ 利用率 + 优先级违反度 + 拖期（另列）（软目标仅利用率，不做帕累托/超体积强主张）
...
错峰到达：50 lot 按 Poisson 到达（指数间隔，均值 arrival_mean_gap，默认 0.6；lot1 在 t=0），
  ...
  "现在派 vs 等下一个更优 lot"成为有意义的决策（喂 DDT）。
...
2. 确立 **Q-time > priority > 利用率** 的字典序结构（Q-time 为阶段间队列时间硬约束；拖期为独立的交付期指标，不进奖励）：Q-time 用**机会约束 mask**（随机加工时间下 `deadline−μ<z_ε·σ`，违规概率≤ε）在决策时硬挡，并以 **PPO-Lagrangian（CMDP/Safe RL 框架，λ 自适应对偶上升）** 在训练目标层面压住残差违规；priority 用候选池过滤（+DDT 预留）、利用率为唯一软目标；
