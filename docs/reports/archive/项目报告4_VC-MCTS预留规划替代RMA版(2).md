...
> 上图把原先 DDT 的"是否 hold"、以及上一版学习式 RMA 的"预留概率"，改成 **VC-MCTS 的搜索式目标绑定预留**：规划器不在所有空闲点都决策，而是在宽触发式 ROP 发现"可能有预留价值"的场景后建树，由对 reserve 与 dispatch/no_op 分支的 rollout 模拟比较决定是否预留、留给谁。**当前代码已通过 `dispatch_delegate.py` 完成 SAS policy 接入**（详见 §7.6）：`vc_mcts_planner.py` 在启用 `dispatch_delegate` 时只比较 `reserve / no_op / delegate_dispatch`，具体 `(lot,ppid)` 派工交给 `RuleDispatchDelegate` 或 `SASPolicyDispatchDelegate`；`--dispatch-delegate topk` 保留旧 TopK dispatch 行为作消融对照。这样把上一版 RMA 想用奖励学的反事实收益，直接由仿真精确算出（§5.5/§5.7）。
...
SAS Actor：目标架构中学习当前机器的候选动作选择概率，并可复用为搜索先验/基策略/叶子估值；当前已通过 `SASPolicyDispatchDelegate`（`dispatch_delegate.py`）接入训练好的 SAS checkpoint（如 `pressure_mh_hard.pt`），`--dispatch-delegate sas` 即可启用
...
三机制的融合点就在这一个 `E` 上：**机制 1 决定 E 里哪些边存在、哪些项加进来（`feasible` 与 `λ·ĉ`）；机制 2 决定 E 的鲁棒性成分 `ρ̂_pc` 长什么样；机制 3 决定 E 里每个带帽估计量算得准不准。** 当前代码实现已落地 objective-first 字典序选择与 no_op gating（见 §6.2.2/§7.6），并通过 `dispatch_delegate` 接入 SAS policy；`ρ̂_pc`（优先级-能力鲁棒性项）与 SAS 先验 `p(s,a)` 尚未完整落地，目前 UCT 利用项为：`objective_to_score = -qtime_count_penalty - qtime_total_penalty - O2 + util_weight·util`，`p(s,a)` 由 `action.prior` 近似。
...
```text
ROP 未命中的决策点 → 直接走 dispatch delegate（rule 或 SAS）贪心派工，近乎免费。
ROP 命中           → 才建树，reserve 边只取 S_res 排序 TopB 对，dispatch 合并为单个 delegate_dispatch 边。
```

DyRo-MCTS 不是抛弃学习——它用离线策略当 PUCT 先验 p(s,a) 并引导 rollout，且实证"先验质量越高、搜索结果越好"。当前代码状态：

```text
SAS（已接入 delegate）→ 通过 SASPolicyDispatchDelegate 被 VC-MCTS 复用为 rollout 基策略；
                       先验 p(s,a) 与多头 Critic 叶子截断尚未完整接入（目前仍跑到底）。
dispatch delegate     → 三种模式: topk（旧行为/消融）、rule（规则 FIFO/SPT 等）、sas（训练好的 checkpoint）。
RMA（学不稳）         → 降级为"预留候选提议器"：只向树提供 reserve(m,h) 候选（ROP+S_res TopB），由 MCTS 验证。
（可选）专家迭代      → 拿 MCTS 改进后的决策当标签回训先验 p(s,a)（方法非原创，不作卖点）。
```
...
```text
阶段 1：资源日历环境 + 下层启发式估时器（1.5）+ 候选池 + 结构 mask             ✅ 完成
阶段 2：规则触发 SAS-PPO，仅即时执行通道（标量、单头）                          ✅ 完成
阶段 3：机会约束 qtime mask + priority filter(soft)                            ✅ 完成
阶段 4：向量 reward + 多头 Critic + 逐目标 GAE + PPO-Lagrangian（可选）         ✅ 完成（checkpoint: pressure_mh_hard.pt）
阶段 5：双注意力 SAS 消融                                                      ✅ 完成（MLP + mask pooling 已落地；完整双注意力待消融）
阶段 6（go/no-go）：oracle 预留上界验证 late_hi                                 ✅ GREEN（§7.5 oracle −12.7% O2 噪声均值，3/3 seed）
阶段 7：VC-MCTS 在线预留规划                                                   ✅ 核心已落地（dispatch delegate + reservation ledger + no_op gating）
  • 机制 1（Q-time 准入）：已实现（qtime-first 字典序选择）
  • dispatch delegate：✅ 已落地 topk/rule/sas 三种模式
  • 机制 2（ρ_pc 优先级-能力鲁棒性）：⬜ 尚未落地（目前只有 -O2 价值项）
  • 机制 3（CRN 噪声 rollout）：⬜ 尚未落地（目前 rollout 无 noise 注入）
  • 叶子 Critic 截断（AlphaZero 式）：⬜ 尚未落地（目前跑到底）
阶段 8：多实例 + 公开基准全面评估（VC-MCTS vs SAS-only vs 规则 vs DyRo 无约束）  🔄 进行中
```
...
**当前实现状态（已完成第二阶段 dispatch delegate 结构）。** VC-MCTS 只在 root 层比较 `reserve / no_op / delegate_dispatch`，具体 `(lot,ppid)` 派工交给 delegate。三种 delegate 模式均已落地：

```text
RuleDispatchDelegate(strategy=FIFO/SPT/...)  # 规则派工（--dispatch-delegate rule）
SASPolicyDispatchDelegate(checkpoint)        # 训练好的 SAS policy（--dispatch-delegate sas），
                                             # 非法/wait/padding 时回退到 rule
topk 模式（--dispatch-delegate topk）        # 旧 TopK dispatch 行为，保留作消融对照
```
